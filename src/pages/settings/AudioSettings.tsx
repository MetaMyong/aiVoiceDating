            ...
            <div className="grid grid-cols-1 gap-6">
              <div>
                ...
                <div className="flex gap-2 mt-2">
                  ...
                  <button type="button" className="px-3 py-2 rounded bg-slate-700/60 text-slate-200 hover:bg-slate-600/70 border border-slate-600/50" onClick={()=>refreshDevices({ forceRequestPermission: true })}>새로고침</button>
                </div>
              </div>
              <div>
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-56">
                      <div style={{height:12, background:'rgba(30,41,59,0.8)', borderRadius:6, overflow:'hidden', border:'1px solid rgba(51,65,85,0.6)'}}>
                        {(() => {
                    ...
                    <div className="text-sm font-mono text-slate-300 w-12 text-right">{Math.min(100, Math.round(level*100))}%</div>
                  </div>
                </div>
                <div className="mt-2 text-xs text-slate-400">Threshold (dB)</div>
                <div className="flex items-center gap-3">
                  <input type="range" min={-80} max={0} step={0.5} value={threshold} onChange={async e=>{ const v = Number(e.target.value); setThreshold(v); thresholdRef.current = v; try{ await saveCfg({ threshold: v }); }catch(_){ } }} className="w-full mt-2 accent-teal-500" />
                  <div className="text-sm font-mono w-20 text-right text-slate-300">{threshold} dB</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="relative overflow-hidden bg-gradient-to-br from-slate-900/60 via-slate-800/40 to-slate-900/60 backdrop-blur-xl rounded-3xl shadow-2xl border border-slate-700/50 p-8">
          ...
          <div className="relative max-w-2xl mx-auto">
            <div className="grid grid-cols-1 gap-6">
              ...
              <div>
                <div className="flex items-center justify-between"><div className="text-sm text-slate-400">재생 볼륨</div><div className="text-xs text-slate-400">{Math.round(playbackVolume*100)}%</div></div>
                <input type="range" min={0} max={1} step={0.01} value={playbackVolume} onChange={async e=>{ const v = Number(e.target.value); setPlaybackVolume(v); try{ await saveCfg({ playbackVolume: v }); }catch(_){}}} className="w-full mt-2 accent-teal-500" />
              </div>
            </div>
          </div>
        </section>
